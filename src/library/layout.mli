...
(** This module defines the concept of layout, that is how an event is
    ...
    It is possible to register new layouts through the [register] function.
    Initially, all the layouts defined in this file (except [pattern] and
...
(** The type of layout, the components being:
...
(** [pattern h f r] constructs a layout [(h, f, r')] where [r'] is the rendering
    function defined by the string [r]. The rendering is done by substituting
    the substring of the form ["$(key)"] or ["$(key:pad)"] with their associated values.
    The key/value associations are as defined by the function [Event.bindings].
    The absolute value of pad defines the minimum size of the value when rendered.
    Moreover, if pad is negative padding spaces are added on the left while they
    are added to the right if pad is positive. *)
...
val xml : t
(** The layout initially registered with the name "xml".
    Its format is compatible with the one defined by Apache log4j
    (cf. http://logging.apache.org/log4j/). *)

val log4j : t
(** Bare synonym for [xml] *)

val csv : string -> string list -> t
(** [csv sep l] constructs a layout [([], [], f)] where [f] is the rendering
    function defined by:
    - [sep] the CSV separator;
    - [l] the list of elements (referred to through keys) to output for each
    CSV record, (the available keys are those defined by [Event.bindings]). *)
